...
## 🧠 Part 2: The Optimal Two-Pointer Mental Model (In Plain English)

To optimize space to $\mathcal{O}(1)$, we drop the arrays and use two moving pointers (`left` and `right`) to track the max boundaries dynamically.
...
## 💻 The Code ($\mathcal{O}(1)$ Space - Two Pointers)
